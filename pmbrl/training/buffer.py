...
class Buffer(object):
    def __init__(
        ...
        self.device = device
        self.transporter = transporter.to(device)
    ...
    def add(self, state, action, reward, next_state):
        ...
        state = self.transporter.get_keypoint(s)['centers'].reshape(-1, 240).detach().numpy()

        s = torch.as_tensor(
            next_state, device=self.device, dtype=torch.float32).reshape(-1, 48, 48, 3)  # type: ignore
        next_state = self.transporter.get_keypoint(s)['centers'].reshape(-1, 240).detach().numpy()
