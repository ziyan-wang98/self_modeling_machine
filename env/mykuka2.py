...
class KukaPAP(KukaGymEnv):
    ...
    def reset(self):
        ...
        urdflist = self._get_random_object(num_objects=5, test=self._isTest)
    ...
    def place_a_objects(self, urdf):
        ...
        urdf_path = os.path.join(self._urdfRoot, urdf)
    ...
    def _step_continuous(self, action):
        ...
        self._env_step += 1
        if not self._attempted_grasp:
            self._kuka.applyAction(action)
        else:
            keep_action = action[:4] + [0]
            self._kuka.applyAction(keep_action)
        ...
        if not self._attempted_grasp and end_effector_pos[2] <= 0.1:
            finger_angle = 0.3
            for _ in range(100):
                ...
                if finger_angle < 0:
                    finger_angle = 0
            end_pos = p.getLinkState(self._kuka.kukaUid, self._kuka.kukaEndEffectorIndex)
            print(1, end_pos[0])
            for _ in range(100):
                grasp_action = [0, 0, 0.02, 0, finger_angle]
                self._kuka.applyAction(grasp_action)
                p.stepSimulation()
                if self._renders:
                    time.sleep(self._timeStep)
        ...
        debug = {'grasp_success': self._placeSuccess}
    ...
    def _reward(self):
        ...
        if not self._firstTouch:
            ...
            for uid in self._objectUids:
                ...
                if dis < 0.1:
                    print("great, U are very close to it")
                    reward += 1
    ...
    def _termination(self):
        ...
        maxSteps steps.
        """
        return self._attempted_place or self._env_step >= self._maxSteps
    ...
    def _get_a_object(self, test):
        ...
        if test:
            ...
            urdf_pattern = os.path.join(self._urdfRoot, 'random_urdfs/*[1-9]/*.urdf')
        found_object_directories = glob.glob(urdf_pattern)
        return found_object_directories[10]
